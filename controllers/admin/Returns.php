...
class MpExportDocumentsReturnsController extends MpCustomController
{
    ...
    public function getFieldsData($params)
    {
        ...
        if ($isStartDate || $isEndDate) {
            $fieldsArray = array(
                'r.id_order_return as document_id',
                'r.date_add as document_date',
                'r.question as return_message',
                'orsl.name as return_state',
            ...
            $sql
                ->from('order_return', '`r`')
                ->innerJoin('orders', '`o`', 'r.id_order=o.id_order')
                ->innerJoin('customer', '`c`', 'c.id_customer=o.id_customer')
                ->innerJoin('order_return_state_lang', 'orsl', 'orsl.id_order_return_state=r.state')
                ->where('orsl.id_lang='.(int) Context::getContext()->language->id)
                ->orderBy('r.date_add');

            $sql = $this->addDateToSql($sql, 'r.date_add');
